...
function parseColumnVector(input) {
    const inner = stripBrackets(input);
    if (inner.length === 0) {
...
function assertSquare(M, name) {
    if (M.length === 0 || M.length !== M[0].length) {
        throw new Error(`${name} must be a square matrix`);
    }
}
function assertShape(M, rows, cols, name) {
...
function solveRiccati(A, B, Q, Qf, R, N) {
...
function simulateTrajectory(A, B, x0, K, N) {
...
async function renderPhasePlot(xs, ys, steps, title) {
    const container = getEl("phase-plot");
...
async function renderCostPlot(horizons, costs, selectedN, selectedCost, penalized) {
    const container = getEl("cost-plot");
...
function update(inputs) {
    ...
    getEl("alpha-field").hidden = !penalized;
    try {
    ...
    try {
        ...
        const costAt = (k) => matMul(matMul(x0T, P[k]), x0)[0][0];
